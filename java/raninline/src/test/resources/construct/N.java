...
public class N {
    ...
    public static long getUInt(final byte... bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            ...
            if (i < bytes.length - 1) {
                res <<= 8;
                itest("Randoop").given(res, 256L).checkEq(res, 65536L);
                itest("Randoop").given(res, 0L).checkEq(res, 0L);
                itest("Randoop").given(res, 7170742L).checkEq(res, 1835709952L);
                itest("Randoop").given(res, 15L).checkEq(res, 3840L);
                itest("Randoop").given(res, 512L).checkEq(res, 131072L);
                itest("Randoop").given(res, 2L).checkEq(res, 512L);
                itest("Randoop").given(res, 1L).checkEq(res, 256L);
                itest("Randoop").given(res, 109L).checkEq(res, 27904L);
                itest("Randoop").given(res, 28010L).checkEq(res, 7170560L);
                itest("Randoop").given(res, 3841L).checkEq(res, 983296L);
            }
        }
        return res;
    }
