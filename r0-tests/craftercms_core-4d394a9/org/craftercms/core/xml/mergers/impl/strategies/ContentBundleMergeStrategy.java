    ...
    public List<MergeableDescriptor> getDescriptors(Context context, CachingOptions cachingOptions, String mainDescriptorUrl, Document mainDescriptorDom, boolean mainDescriptorOptional) throws XmlMergeException {
        ...
        if (prefix.length() < mainDescriptorUrl.length()) {
            ...
            while (delimiterIdx > 0 && !baseFound) {
                ...
                itest("dev", 103).given(delimiterIdx, 7).given(baseName, "folder2_es").checkEq(baseName, "folder2");
                itest("dev", 103).given(delimiterIdx, 7).given(baseName, "content_fr").checkEq(baseName, "content");
            ...
            itest("dev", 133).given(prefix, "/folder1/").given(mainDescriptorUrl, "/folder1/folder2_es/file.xml").checkEq(noPrefix, "folder2_es/file.xml");
            itest("dev", 133).given(prefix, "/bundle/").given(mainDescriptorUrl, "/bundle/content_fr_es/descriptor.xml").checkEq(noPrefix, "content_fr_es/descriptor.xml");
            itest("dev", 133).given(prefix, "/bundle/").given(mainDescriptorUrl, "/bundle/content_fr/descriptor.xml").checkEq(noPrefix, "content_fr/descriptor.xml");
