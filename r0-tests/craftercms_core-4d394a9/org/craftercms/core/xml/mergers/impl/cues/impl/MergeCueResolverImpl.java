    ...
    protected Map<String, String> getMergeCueParams(Element element, Attribute mergeCueAttribute) {
        ...
        for (Iterator<Attribute> i = attributes.iterator(); i.hasNext(); ) {
            ...
            if (attributeQualifiedName.startsWith(paramsPrefix)) {
                ...
                itest("evosuite", 134).given(paramsPrefix, "-").given(attributeQualifiedName, "-").checkEq(paramName, "");
                itest("dev", 134).given(paramsPrefix, "merge-with-parent-").given(attributeQualifiedName, "merge-with-parent-order").checkEq(paramName, "order");
                itest("dev", 134).given(paramsPrefix, "hp-").given(attributeQualifiedName, "hp-param").checkEq(paramName, "param");
                itest("dev", 134).given(paramsPrefix, "merge-with-child-").given(attributeQualifiedName, "merge-with-child-order").checkEq(paramName, "order");
