...
public class ID3v2CommentFrameData extends AbstractID3v2FrameData {
    ...
    protected byte[] packFrameData() {
        ...
        if (language == null) {
            ...
            itest("evosuite", 59).given(language, "X1!ggJ").checkEq(langPadded, "X1!");
            itest("randoop", 59).given(language, "M/S stereo").checkEq(langPadded, "M/S");
            itest("evosuite", 59).given(language, "BigHW`.&Atvd^ot").checkEq(langPadded, "Big");
            itest("evosuite", 59).given(language, "p&m@4nVS*S)U").checkEq(langPadded, "p&m");
            itest("evosuite", 59).given(language, "m?>,[)fm0Xk$").checkEq(langPadded, "m?>");
            itest("evosuite", 59).given(language, "0;*Hj7RG").checkEq(langPadded, "0;*");
            itest("evosuite", 59).given(language, "[%U=n=$-!3h%}@k").checkEq(langPadded, "[%U");
            itest("evosuite", 59).given(language, "UTF-16LE").checkEq(langPadded, "UTF");
            itest("evosuite", 59).given(language, "UTF-16BE").checkEq(langPadded, "UTF");
            itest("randoop", 59).given(language, "COMM").checkEq(langPadded, "COM");
        } else {
