...
public class RuntimeContainer {
    ...
    private static String getHostname(InetAddress inetAddress, boolean fqdn) throws IOException {
        ...
        if (hostname.indexOf('.') != -1 && !fqdn) {
            itest("randoop", 121).given(hostname, "af98d36bacae").given(fqdn, true).checkFalse(group());
            itest("dev", 121).given(hostname, "af98d36bacae").given(fqdn, false).checkFalse(group());
            itest("dev", 121).given(hostname, "af98d36bacae").given(fqdn, true).checkFalse(group());
            itest("randoop", 121).given(hostname, "af98d36bacae").given(fqdn, false).checkFalse(group());
