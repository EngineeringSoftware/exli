...
public class KafkaGelfSenderProvider implements GelfSenderProvider {
    ...
    private static String getBrokerServers(GelfSenderConfiguration configuration) {
        ...
        itest("dev", 82).given(scheme, "kafka://").given(hconf, "kafka://localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2#").checkEq(hostsPart, "localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2#");
        itest("dev", 82).given(scheme, "kafka://").given(hconf, "kafka://localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2").checkEq(hostsPart, "localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2");
        ...
        itest("dev", 87).given(pos, 29).given(hostsPart, "localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2").checkEq(suffix, "?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2");
        itest("dev", 87).given(pos, 29).given(hostsPart, "localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2#").checkEq(suffix, "?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2#");
        hostsPart = hostsPart.substring(0, pos);
        itest("dev", 88).given(pos, 29).given(hostsPart, "localhost:9092,localhost:9093?acks=1&ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks&retries=2").checkEq(hostsPart, "localhost:9092,localhost:9093");
        ...
        if (hostsPart.length() > 0) {
            ...
            itest("dev", 93).given(hostsPart, "localhost:9092,localhost:9093").checkEq(hosts, new String[] { "localhost:9092", "localhost:9093" });
        }
        if (hosts.length > 0) {
