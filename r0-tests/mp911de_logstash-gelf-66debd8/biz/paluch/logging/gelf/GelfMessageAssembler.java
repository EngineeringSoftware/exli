...
public class GelfMessageAssembler implements HostAndPortProvider {
    ...
    private static final Set<NamedLogField> SOURCE_FIELDS = EnumSet.of(NamedLogField.SourceClassName, NamedLogField.SourceSimpleClassName, NamedLogField.SourceMethodName, NamedLogField.SourceLineNumber);
    ...
    private void setupStaticFields(PropertyProvider propertyProvider) {
        ...
        while (true) {
            ...
            itest("dev", 245).given(property, "fieldName3=fieldValue3").checkEq(index, 10);
            itest("dev", 245).given(property, "fieldName4=fieldValue4").checkEq(index, 10);
            if (-1 != index) {
                StaticMessageField field = new StaticMessageField(property.substring(0, index), property.substring(index + 1));
                itest("dev", 248).given(property, "fieldName4=fieldValue4").given(index, 10).checkEq(field, "8.xml");
                itest("dev", 248).given(property, "fieldName3=fieldValue3").given(index, 10).checkEq(field, "7.xml");
                addField(field);
            }
            fieldNumber++;
        }
    ...
    public void setMaximumMessageSize(int maximumMessageSize) {
        if (maximumMessageSize > MAX_MESSAGE_SIZE || maximumMessageSize < 1) {
            throw new IllegalArgumentException("Invalid maximum message size: " + maximumMessageSize + ", supported range: 1-" + MAX_MESSAGE_SIZE);
        }
    ...
    public void setVersion(String version) {
        if (!GelfMessage.GELF_VERSION_1_0.equals(version) && !GelfMessage.GELF_VERSION_1_1.equals(version)) {
            throw new IllegalArgumentException("Invalid GELF version: " + version + ", supported range: " + GelfMessage.GELF_VERSION_1_0 + ", " + GelfMessage.GELF_VERSION_1_1);
        }
        this.version = version;
    }
