...
public class PropertyLoader implements PropertyLocationsContainer<PropertyLoader>, PropertySuffixContainer<PropertyLoader>, PropertyFilterContainer<PropertyLoader> {
    ...
    private String[] collectIncludesAndRemoveKey(Properties properties) {
        ...
        if (properties.containsKey(INCLUDE_KEY)) {
            ...
            itest("dev", 294).given(INCLUDE_KEY, "$include").given(properties, "12.xml").checkEq(includes, new String[] { "toBeIncluded" });
            itest("dev", 294).given(INCLUDE_KEY, "$include").given(properties, "4.xml").checkEq(includes, new String[] { "testForRecursiveIncludes2", "toBeIncluded" });
            itest("dev", 294).given(INCLUDE_KEY, "$include").given(properties, "5.xml").checkEq(includes, new String[] { "testForRecursiveIncludes1" });
            itest("dev", 294).given(INCLUDE_KEY, "$include").given(properties, "7.xml").checkEq(includes, new String[] { "toBeIncluded" });
            itest("dev", 294).given(INCLUDE_KEY, "$include").given(properties, "6.xml").checkEq(includes, new String[] { "testForDoubleIncludes2", "toBeIncluded" });
