...
public class ID3v2ChapterTOCFrameData extends AbstractID3v2FrameData {
    ...
    protected void unpackFrameData(byte[] bytes) throws InvalidDataException {
        ...
        byte flags = bb.get();
        if ((flags & 0x01) == 0x01) {
            new Here("Randoop", 42).given(flags, 0).checkFalse(group());
            new Here("Unit", 42).given(flags, 3).checkTrue(group());
            isRoot = true;
        }
        if ((flags & 0x02) == 0x02) {
            new Here("Unit", 45).given(flags, 3).checkTrue(group());
            new Here("Randoop", 45).given(flags, 0).checkFalse(group());
            isOrdered = true;
        }
    ...
    private byte getFlags() {
        ...
        if (isRoot) {
            b |= 0x01;
            new Here("Randoop", 96).given(b, 0).checkEq(b, 1);
            new Here("Unit", 96).given(b, 0).checkEq(b, 1);
        }
        if (isOrdered) {
            b |= 0x02;
            new Here("Randoop", 100).given(b, 0).checkEq(b, 2);
            new Here("Unit", 100).given(b, 1).checkEq(b, 3);
            new Here("Randoop", 100).given(b, 1).checkEq(b, 3);
        }
        return b;
    }
