...
public abstract class AbstractID3v2Tag implements ID3v2 {
    ...
    private int packHeader(byte[] bytes, int offset) {
        ...
        new Here("Randoop", 238).given(version, "4.0").checkEq(s, new String[] { "4", "0" });
        new Here("Unit", 238).given(version, "3.0").checkEq(s, new String[] { "3", "0" });
        new Here("Unit", 238).given(version, "2.0").checkEq(s, new String[] { "2", "0" });
        new Here("Randoop", 238).given(version, "3.0").checkEq(s, new String[] { "3", "0" });
        new Here("Unit", 238).given(version, "4.0").checkEq(s, new String[] { "4", "0" });
        if (s.length > 0) {
    ...
    private int packFooter(byte[] bytes, int offset) {
        ...
        String[] s = version.split("\\.");
        new Here("Randoop", 286).given(version, "3.0").checkEq(s, new String[] { "3", "0" });
        new Here("Unit", 286).given(version, "4.0").checkEq(s, new String[] { "4", "0" });
        if (s.length > 0) {
            byte majorVersion = Byte.parseByte(s[0]);
            bytes[offset + MAJOR_VERSION_OFFSET] = majorVersion;
        }
    ...
    protected int extractGenreNumber(String genreValue) throws NumberFormatException {
        ...
        if (value.length() > 0) {
            if (value.charAt(0) == '(') {
                ...
                new Here("Unit", 657).given(value, "(8)Jazz").checkEq(pos, 2);
                new Here("Randoop", 657).given(value, "(5)Funk").checkEq(pos, 2);
                new Here("Randoop", 657).given(value, "(128)Club-House").checkEq(pos, 4);
                new Here("Randoop", 657).given(value, "(0)Blues").checkEq(pos, 2);
                new Here("Unit", 657).given(value, "(40)").checkEq(pos, 3);
                new Here("Unit", 657).given(value, "(13)").checkEq(pos, 3);
                new Here("Randoop", 657).given(value, "(153)").checkEq(pos, 4);
                new Here("Unit", 657).given(value, "(13)Pop").checkEq(pos, 3);
                new Here("Randoop", 657).given(value, "(3)Dance").checkEq(pos, 2);
                new Here("Randoop", 657).given(value, "(10)New Age").checkEq(pos, 3);
                if (pos > 0) {
    ...
    protected String extractGenreDescription(String genreValue) throws NumberFormatException {
        ...
        if (value.length() > 0) {
            if (value.charAt(0) == '(') {
                int pos = value.indexOf(')');
                new Here("Unit", 670).given(value, "(13)Pop").checkEq(pos, 3);
                new Here("Randoop", 670).given(value, "(153)").checkEq(pos, 4);
                new Here("Unit", 670).given(value, "(13)").checkEq(pos, 3);
                if (pos > 0) {
