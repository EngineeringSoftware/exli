...
class Proc implements EventSink {
    ...
    private static String naiveShellEscape(String arg) {
        ...
        String escapedArg;
        if (m.find()) {
            new Here("Unit", 205).given(m, "1.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "9.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "18.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "14.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "10.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "16.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "12.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "4.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "6.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "2.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "11.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "0.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "22.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "19.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "13.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "8.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "5.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "20.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "17.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "21.xml").checkFalse(group());
            new Here("Unit", 205).given(m, "7.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "3.xml").checkTrue(group());
            new Here("Unit", 205).given(m, "15.xml").checkFalse(group());
