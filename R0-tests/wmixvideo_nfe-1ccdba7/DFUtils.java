...
public abstract class DFUtils {
    ...
    public static boolean isCnpjValido(final String cnpj) {
        if (cnpj == null || !cnpj.matches("^[0-9]{14}$")) {
            new Here("Unit", 20).given(cnpj, "00038883975022").checkFalse(group());
            new Here("Randoop", 20).given(cnpj, "Ferrovi\u00E1rio").checkTrue(group());
            new Here("Randoop", 20).given(cnpj, "110140").checkTrue(group());
            new Here("Unit", 20).given(cnpj, "00000000000000").checkFalse(group());
            new Here("Unit", 20).given(cnpj, null).checkTrue(group());
            new Here("Randoop", 20).given(cnpj, "PR").checkTrue(group());
            new Here("Unit", 20).given(cnpj, "03918609000132").checkFalse(group());
            new Here("Unit", 20).given(cnpj, "11111111111111").checkFalse(group());
            new Here("Unit", 20).given(cnpj, "").checkTrue(group());
            return false;
        }
    ...
    public static boolean isCpfValido(final String cpf) {
        if (cpf == null || !cpf.matches("^[0-9]{11}$")) {
            new Here("Unit", 72).given(cpf, "11111111111").checkFalse(group());
            new Here("Randoop", 72).given(cpf, "https://nfce.sefaz.ms.gov.br/ws/NFeRecepcaoEvento4").checkTrue(group());
            new Here("Unit", 72).given(cpf, "18609000132").checkFalse(group());
            new Here("Unit", 72).given(cpf, "38883975022").checkFalse(group());
            new Here("Unit", 72).given(cpf, "12345678909").checkFalse(group());
            new Here("Unit", 72).given(cpf, "00000000000").checkFalse(group());
            new Here("Unit", 72).given(cpf, "").checkTrue(group());
            new Here("Randoop", 72).given(cpf, "Rodovi\u00E1rio").checkTrue(group());
            new Here("Randoop", 72).given(cpf, "https://nfce.sefa.pr.gov.br/nfce/NFeConsultaProtocolo4?wsdl").checkTrue(group());
            new Here("Unit", 72).given(cpf, null).checkTrue(group());
            return false;
        }
        // verifica por falsos positivos
        ...
        new Here("Unit", 107).given(digito1, 1).given(digito2, 7).checkEq(nDigResult, "17");
        new Here("Unit", 107).given(digito1, 2).given(digito2, 2).checkEq(nDigResult, "22");
