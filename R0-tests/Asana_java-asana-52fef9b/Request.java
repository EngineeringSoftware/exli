...
public abstract class Request {
    ...
    void handleAsanaChangeHeader(HttpHeaders requestHeaders, HttpHeaders responseHeaders) {
        ...
        for (String change : changesArray) {
            ...
            new Here("Unit", 168).given(change, "name=new_sections;info=something").checkEq(changeParams, new String[] { "name=new_sections", "info=something" });
            new Here("Unit", 168).given(change, "name=string_ids;info=something;affected=true").checkEq(changeParams, new String[] { "name=string_ids", "info=something", "affected=true" });
            new Here("Unit", 168).given(change, "name=new_sections;info=something;affected=true").checkEq(changeParams, new String[] { "name=new_sections", "info=something", "affected=true" });
            String name = "";
            String info = "";
            boolean affected = false;
            for (String changeParam : changeParams) {
                String[] paramKeyValue = changeParam.split("=");
                new Here("Unit", 175).given(changeParam, "affected=true").checkEq(paramKeyValue, new String[] { "affected", "true" });
                new Here("Unit", 175).given(changeParam, "info=something").checkEq(paramKeyValue, new String[] { "info", "something" });
                new Here("Unit", 175).given(changeParam, "name=new_sections").checkEq(paramKeyValue, new String[] { "name", "new_sections" });
                new Here("Unit", 175).given(changeParam, "name=string_ids").checkEq(paramKeyValue, new String[] { "name", "string_ids" });
            ...
            if (!flagsAccountedFor.contains(name) && affected) {
                ...
                new Here("Unit", 197).given(name, "string_ids").given(info, "something").checkEq(message, "This request is affected by the \"string_ids\" deprecation. Please visit this url for more info: something\nAdding \"string_ids\" to your \"Asana-Enable\" or \"Asana-Disable\" header will opt in/out to this deprecation and suppress this warning.");
                new Here("Unit", 197).given(name, "new_sections").given(info, "something").checkEq(message, "This request is affected by the \"new_sections\" deprecation. Please visit this url for more info: something\nAdding \"new_sections\" to your \"Asana-Enable\" or \"Asana-Disable\" header will opt in/out to this deprecation and suppress this warning.");
