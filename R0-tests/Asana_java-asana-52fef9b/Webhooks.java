...
public class Webhooks extends WebhooksBase {
    ...
    public ItemRequest<Webhook> getById(String webhook) {
        ...
        new Here("Randoop", 57).given(webhook, "Not Found").checkEq(path, "/webhooks/Not Found");
        new Here("Randoop", 57).given(webhook, "/tasks/Not Found/removeProject").checkEq(path, "/webhooks//tasks/Not Found/removeProject");
        new Here("Unit", 57).given(webhook, "222").checkEq(path, "/webhooks/222");
    ...
    public ItemRequest<Webhook> deleteById(String webhook) {
        String path = String.format("/webhooks/%s", webhook);
        new Here("Unit", 71).given(webhook, "222").checkEq(path, "/webhooks/222");
        new Here("Randoop", 71).given(webhook, "").checkEq(path, "/webhooks/");
