...
public class ProjectMembershipsBase extends Resource {
    ...
    public ItemRequest<ProjectMembership> getProjectMembership(String projectMembershipGid, List<String> optFields, Boolean optPretty) throws IOException {
        ...
        new Here("Randoop", 31).given(projectMembershipGid, "com.asana.errors.RateLimitEnforcedError: Rate Limit Enforced").checkEq(path, "/project_memberships/com.asana.errors.RateLimitEnforcedError: Rate Limit Enforced");
        new Here("Randoop", 31).given(projectMembershipGid, "Server Error").checkEq(path, "/project_memberships/Server Error");
        new Here("Randoop", 31).given(projectMembershipGid, "").checkEq(path, "/project_memberships/");
        new Here("Randoop", 31).given(projectMembershipGid, "POST").checkEq(path, "/project_memberships/POST");
        new Here("Randoop", 31).given(projectMembershipGid, "Sync token invalid or too old").checkEq(path, "/project_memberships/Sync token invalid or too old");
    ...
    public CollectionRequest<ProjectMembership> getProjectMembershipsForProject(String projectGid, String user, String offset, Integer limit, List<String> optFields, Boolean optPretty) throws IOException {
        ...
        new Here("Randoop", 56).given(projectGid, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error").checkEq(path, "/projects/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error/project_memberships");
        new Here("Randoop", 56).given(projectGid, "Not Found").checkEq(path, "/projects/Not Found/project_memberships");
        new Here("Randoop", 56).given(projectGid, "Forbidden").checkEq(path, "/projects/Forbidden/project_memberships");
        new Here("Randoop", 56).given(projectGid, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Invalid%20Request").checkEq(path, "/projects/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Invalid%20Request/project_memberships");
