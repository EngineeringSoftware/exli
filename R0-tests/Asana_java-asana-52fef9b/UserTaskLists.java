...
public class UserTaskLists extends UserTaskListsBase {
    ...
    public ItemRequest<UserTaskList> findByUser(String user) {
        ...
        new Here("Randoop", 24).given(user, "hi!").checkEq(path, "/users/hi!/user_task_list");
        new Here("Randoop", 24).given(user, "/tasks/Not Found/removeProject").checkEq(path, "/users//tasks/Not Found/removeProject/user_task_list");
        new Here("Randoop", 24).given(user, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=com.asana.errors.RateLimitEnforcedError:%20Rate%20Limit%20Enforced").checkEq(path, "/users/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=com.asana.errors.RateLimitEnforcedError:%20Rate%20Limit%20Enforced/user_task_list");
    ...
    public ItemRequest<UserTaskList> findById(String userTaskList) {
        ...
        new Here("Randoop", 36).given(userTaskList, "Forbidden").checkEq(path, "/user_task_lists/Forbidden");
        return new ItemRequest<UserTaskList>(this, UserTaskList.class, path, "GET");
    }
    ...
    public CollectionRequest<UserTaskList> tasks(String userTaskList) {
        ...
        new Here("Randoop", 64).given(userTaskList, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error").checkEq(path, "/user_task_lists/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error/tasks");
        new Here("Randoop", 64).given(userTaskList, "hi!").checkEq(path, "/user_task_lists/hi!/tasks");
        new Here("Randoop", 64).given(userTaskList, "urn:ietf:wg:oauth:2.0:oob").checkEq(path, "/user_task_lists/urn:ietf:wg:oauth:2.0:oob/tasks");
