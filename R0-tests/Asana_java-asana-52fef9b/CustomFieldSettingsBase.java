...
public class CustomFieldSettingsBase extends Resource {
    ...
    public CollectionRequest<CustomFieldSetting> getCustomFieldSettingsForPortfolio(String portfolioGid, String offset, Integer limit, List<String> optFields, Boolean optPretty) throws IOException {
        ...
        new Here("Randoop", 33).given(portfolioGid, "200").checkEq(path, "/portfolios/200/custom_field_settings");
        new Here("Randoop", 33).given(portfolioGid, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error").checkEq(path, "/portfolios/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error/custom_field_settings");
    ...
    public CollectionRequest<CustomFieldSetting> getCustomFieldSettingsForProject(String projectGid, String offset, Integer limit, List<String> optFields, Boolean optPretty) throws IOException {
        ...
        new Here("Randoop", 59).given(projectGid, "urn:ietf:wg:oauth:2.0:oob").checkEq(path, "/projects/urn:ietf:wg:oauth:2.0:oob/custom_field_settings");
        new Here("Randoop", 59).given(projectGid, "").checkEq(path, "/projects//custom_field_settings");
        new Here("Randoop", 59).given(projectGid, "/tasks/Not Found/removeProject").checkEq(path, "/projects//tasks/Not Found/removeProject/custom_field_settings");
        new Here("Randoop", 59).given(projectGid, "https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error").checkEq(path, "/projects/https://app.asana.com/-/oauth_authorize?client_id&redirect_uri=hi!&response_type=code&state=Server%20Error/custom_field_settings");
        new Here("Randoop", 59).given(projectGid, "hi!").checkEq(path, "/projects/hi!/custom_field_settings");
        new Here("Randoop", 59).given(projectGid, "Server Error").checkEq(path, "/projects/Server Error/custom_field_settings");
        CollectionRequest<CustomFieldSetting> req = new CollectionRequest<CustomFieldSetting>(this, CustomFieldSetting.class, path, "GET").query("opt_pretty", optPretty).query("opt_fields", optFields).query("limit", limit).query("offset", offset);
        return req;
    }
