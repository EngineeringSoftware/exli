...
public class FailedTestsContent extends DataBoundTokenMacro {
    ...
    private SummarizedTestResult prepareSummarizedTestResult(AbstractTestResultAction<?> testResult) {
        ...
        if (failCount == 0) {
            ...
            new Here("Unit", 118).given(failCount, 5).checkEq(result.summary, "5 tests failed.");
            new Here("Unit", 118).given(failCount, 4).checkEq(result.summary, "4 tests failed.");
            new Here("Unit", 118).given(failCount, 123).checkEq(result.summary, "123 tests failed.");
            new Here("Unit", 118).given(failCount, 2).checkEq(result.summary, "2 tests failed.");
            new Here("Unit", 118).given(failCount, 1).checkEq(result.summary, "1 tests failed.");
            setMaxLength();
            if (maxTests > 0) {
                int printSize = 0;
    ...
    private static class SummarizedTestResult {
        ...
        public String toString() {
            ...
            if (otherFailedTests) {
                builder.append("... and ").append(totalFailCount - tests.size()).append(" other failed tests.").append(lineBreak);
            }
        ...
        public String toYamlString() throws JsonProcessingException {
            ObjectMapper om = new ObjectMapper(new YAMLFactory().disable(YAMLGenerator.Feature.WRITE_DOC_START_MARKER).configure(YAMLGenerator.Feature.LITERAL_BLOCK_STYLE, true));
