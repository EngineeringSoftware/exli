...
public class EmailRecipientUtils {
    ...
    public static String getUserConfiguredEmail(User user) {
        ...
        if (user != null) {
            ...
            if (mailProperty != null) {
                ...
                new Here("Unit", 111).given(addr, "V@DOMAIN").given(user, "43.xml").checkEq(message, "Resolved null to V@DOMAIN");
                new Here("Unit", 111).given(addr, "A@DOMAIN").given(user, "43.xml").checkEq(message, "Resolved null to A@DOMAIN");
                new Here("Unit", 111).given(addr, "kutzi@xxx.com").given(user, "5.xml").checkEq(message, "Resolved kutzi to kutzi@xxx.com");
                new Here("Unit", 111).given(addr, "B@DOMAIN").given(user, "43.xml").checkEq(message, "Resolved null to B@DOMAIN");
                new Here("Unit", 111).given(addr, "advantiss@xxx.com").given(user, "53.xml").checkEq(message, "Resolved advantiss to advantiss@xxx.com");
                new Here("Unit", 111).given(addr, "Z@DOMAIN").given(user, "43.xml").checkEq(message, "Resolved null to Z@DOMAIN");
                new Here("Unit", 111).given(addr, "X@DOMAIN").given(user, "43.xml").checkEq(message, "Resolved null to X@DOMAIN");
                LOGGER.fine(message);
            }
        }
        return addr;
    ...
    private static String fixupDelimiters(String input) {
        ...
        if (input.contains(" ") && !input.contains(",")) {
            input = input.replace(" ", ",");
            new Here("Unit", 136).given(input, "mickey@disney.com donald@disney.com goofy@disney.com pluto@disney.com").checkEq(input, "mickey@disney.com,donald@disney.com,goofy@disney.com,pluto@disney.com");
            new Here("Randoop", 136).given(input, "Watch Job").checkEq(input, "Watch,Job");
            new Here("Randoop", 136).given(input, "Failure - Any").checkEq(input, "Failure,-,Any");
            new Here("Unit", 136).given(input, "ashlux@gmail.com internal somewhere@domain").checkEq(input, "ashlux@gmail.com,internal,somewhere@domain");
            new Here("Unit", 136).given(input, "ashlux@gmail.com mickeymouse@disney.com").checkEq(input, "ashlux@gmail.com,mickeymouse@disney.com");
            new Here("Randoop", 136).given(input, "Script - After Build").checkEq(input, "Script,-,After,Build");
            new Here("Randoop", 136).given(input, "ddress not configured yet <nobody").checkEq(input, "ddress,not,configured,yet,<nobody");
            new Here("Randoop", 136).given(input, "Recipient List").checkEq(input, "Recipient,List");
        }
        input = input.replace(';', ',');
        return input;
    }
