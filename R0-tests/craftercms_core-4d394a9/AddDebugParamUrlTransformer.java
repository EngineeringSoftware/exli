...
public class AddDebugParamUrlTransformer implements UrlTransformer {
    ...
    public String transformUrl(Context context, CachingOptions cachingOptions, String url) {
        ...
        new Here("Randoop", 50).given(url, "-v10379754975141330").checkEq(indexOfParamDelim, -1);
        new Here("Randoop", 50).given(url, "/[null]").checkEq(indexOfParamDelim, -1);
        new Here("Unit", 50).given(url, "/index.html?param=value").checkEq(indexOfParamDelim, 11);
        new Here("Randoop", 50).given(url, "/contentStoreAdapter.item").checkEq(indexOfParamDelim, -1);
        new Here("Unit", 50).given(url, "/index.html").checkEq(indexOfParamDelim, -1);
        new Here("Randoop", 50).given(url, "Tree[name='null', url='null', descriptorUrl='null', properties=null, children=[]]").checkEq(indexOfParamDelim, -1);
