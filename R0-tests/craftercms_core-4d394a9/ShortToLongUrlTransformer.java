...
public class ShortToLongUrlTransformer implements UrlTransformer {
        ...
        try {
            ...
            if (CollectionUtils.isNotEmpty(items)) {
                for (Item item : items) {
                    ...
                    if (UrlUtils.getShortName(itemName, containsShortNameRegex, shortNameRegexGroup).equalsIgnoreCase(shortName)) {
                        return itemName;
                    }
                }
            }
    ...
    protected String getLongUrl(Context context, CachingOptions cachingOptions, String shortUrl, boolean useShortNameIfLongNameNotFound) throws UrlTransformationException {
        ...
        new Here("Randoop", 94).given(shortUrl, "UrlTransformationException: hi!-v-1").checkEq(levels, new String[] { "UrlTransformationException: hi!-v-1" });
        new Here("Unit", 94).given(shortUrl, "/folder/base_fr_es/index.html/").checkEq(levels, new String[] { "folder", "base_fr_es", "index.html" });
        new Here("Randoop", 94).given(shortUrl, "item-list").checkEq(levels, new String[] { "item-list" });
        new Here("Randoop", 94).given(shortUrl, "Cache-Control").checkEq(levels, new String[] { "Cache-Control" });
        new Here("Unit", 94).given(shortUrl, "/folder/subfolder/index.html/").checkEq(levels, new String[] { "folder", "subfolder", "index.html" });
        new Here("Unit", 94).given(shortUrl, "/folder/base/index.html/").checkEq(levels, new String[] { "folder", "base", "index.html" });
        new Here("Randoop", 94).given(shortUrl, "3a00e5a8-1e2c-43b5-9aaa-4af1e5b737af").checkEq(levels, new String[] { "3a00e5a8-1e2c-43b5-9aaa-4af1e5b737af" });
        new Here("Unit", 94).given(shortUrl, "/folder/base_fr/index.html/").checkEq(levels, new String[] { "folder", "base_fr", "index.html" });
        new Here("Randoop", 94).given(shortUrl, "UTF-8").checkEq(levels, new String[] { "UTF-8" });
        new Here("Randoop", 94).given(shortUrl, "hi!-v-1").checkEq(levels, new String[] { "hi!-v-1" });
