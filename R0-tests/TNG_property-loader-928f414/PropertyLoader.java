...
public class PropertyLoader implements PropertyLocationsContainer<PropertyLoader>, PropertySuffixContainer<PropertyLoader>, PropertyFilterContainer<PropertyLoader> {
    ...
    private String[] collectIncludesAndRemoveKey(Properties properties) {
        ...
        if (properties.containsKey(INCLUDE_KEY)) {
            ...
            new Here("Unit", 294).given(INCLUDE_KEY, "$include").given(properties, "5.xml").checkEq(includes, new String[] { "toBeIncluded" });
            new Here("Unit", 294).given(INCLUDE_KEY, "$include").given(properties, "4.xml").checkEq(includes, new String[] { "testForDoubleIncludes2", "toBeIncluded" });
            new Here("Unit", 294).given(INCLUDE_KEY, "$include").given(properties, "2.xml").checkEq(includes, new String[] { "testForRecursiveIncludes2", "toBeIncluded" });
            new Here("Unit", 294).given(INCLUDE_KEY, "$include").given(properties, "3.xml").checkEq(includes, new String[] { "testForRecursiveIncludes1" });
            new Here("Unit", 294).given(INCLUDE_KEY, "$include").given(properties, "10.xml").checkEq(includes, new String[] { "toBeIncluded" });
