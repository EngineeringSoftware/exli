...
public class VariableResolvingFilter extends ValueModifyingFilter {
    ...
    protected String filterValue(String key, String value, Properties properties) {
        ...
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${coresystem.url.dev}").checkEq(endIndex, 20);
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${value}").checkEq(endIndex, 7);
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${a}").checkEq(endIndex, 3);
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${definedInIncluded}").checkEq(endIndex, 19);
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${coresystem.pw.dev}").checkEq(endIndex, 19);
        new Here("Unit", 20).given(startIndex, 0).given(VARIABLE_PREFIX, "${").given(VARIABLE_SUFFIX, "}").given(value, "${testInclude.dev}").checkEq(endIndex, 17);
