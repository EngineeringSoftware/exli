...
public class TimePicker extends FormElement implements LabelableElement, TooltipableElement {
    ...
    private void computeAndPutValidationProperties(Map<String, Object> attributesMapBi) {
        ...
        boolean hasValidation = validationMin || validationMax || validationPattern || validationOptions || validationStrict;
        if (validationMin) {
    ...
    private void assertAttrDisabledTime() throws InvalidInputException {
        ...
        if (disabledTime.length() > DISABLED_TIME_RANGE_MAX_LENGTH) {
            throw new InvalidInputException(String.format("Attribute \"%s\" exceed maximum allowed length (%d)", DISABLED_TIME_ATTR, DISABLED_TIME_RANGE_MAX_LENGTH));
        }
        try {
    ...
    private void assertAttrStepRange() throws InvalidInputException {
        ...
        if (stepValue < MIN_STEP_ALLOWED || stepValue > MAX_STEP_ALLOWED) {
            throw new InvalidInputException(String.format("Attribute \"%s\" must be less than \"%s\" and more than \"%s\".", STEP_ATTR, MAX_STEP_ALLOWED, MIN_STEP_ALLOWED));
        }
    }
    ...
    private void assertAttrFormat() throws InvalidInputException {
        ...
        if (!format.matches(FORMAT_ATTR_PATTERN)) {
            new Here("Unit", 242).given(FORMAT_ATTR_PATTERN, "^[hHmsa: ]+$").given(format, "dd:mm:ss a").checkTrue(group());
            new Here("Unit", 242).given(FORMAT_ATTR_PATTERN, "^[hHmsa: ]+$").given(format, "HH:mm:ss").checkFalse(group());
            new Here("Unit", 242).given(FORMAT_ATTR_PATTERN, "^[hHmsa: ]+$").given(format, "hh:mm").checkFalse(group());
            new Here("Unit", 242).given(FORMAT_ATTR_PATTERN, "^[hHmsa: ]+$").given(format, "hh:mm a").checkFalse(group());
