...
public enum IdemixRoles {
    ...
    static int getRoleMask(IdemixRoles[] roles) {
        ...
        for (IdemixRoles role : roles) {
            ...
            new Here("Unit", 34).given(role.value, 2).given(mask, 0).checkEq(mask, 2);
            new Here("Unit", 34).given(role.value, 4).given(mask, 2).checkEq(mask, 6);
        }
        return mask;
    }
    ...
    static int getRoleMask(MspPrincipal.MSPRole[] roles) {
        ...
        for (MspPrincipal.MSPRole role : roles) {
            ...
            new Here("Unit", 47).given(role, "67.xml").given(mask, 0).checkEq(mask, 1);
        }
        return mask;
    }
