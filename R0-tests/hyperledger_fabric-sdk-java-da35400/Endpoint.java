            ...
            if ("grpcs".equals(protocol)) {
                ...
                try (ByteArrayOutputStream bis = new ByteArrayOutputStream(64000)) {
                    ...
                    if (properties.containsKey("pemFile")) {
                        ...
                        new Here("Unit", 118).given(pemFile, "/home/user/projects/inlinegen-research/_downloads/hyperledger_fabric-sdk-java/src/test/resources/keypair-signed.crt").checkEq(pems, new String[] { "/home/user/projects/inlinegen-research/_downloads/hyperledger_fabric-sdk-java/src/test/resources/keypair-signed.crt" });
                        new Here("Unit", 118).given(pemFile, "src/test/fixture/testPems/caBundled.pems,src/test/fixture/testPems/AnotherUniqCA.pem").checkEq(pems, new String[] { "src/test/fixture/testPems/caBundled.pems", "src/test/fixture/testPems/AnotherUniqCA.pem" });
                        new Here("Unit", 118).given(pemFile, "src/test/fixture/testPems/caBundled.pems,src/test/fixture/testPems/IMBAD,src/test/fixture/testPems/Org1MSP_CA.pem").checkEq(pems, new String[] { "src/test/fixture/testPems/caBundled.pems", "src/test/fixture/testPems/IMBAD", "src/test/fixture/testPems/Org1MSP_CA.pem" });
                        for (String pem : pems) {
    ...
    private void addNettyBuilderProps(NettyChannelBuilder channelBuilder, Properties props) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        for (Map.Entry<?, ?> es : props.entrySet()) {
            ...
            if (match.matches() && match.groupCount() == 1) {
                new Here("Unit", 345).given(match, "49.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "61.xml").checkTrue(group());
                new Here("Unit", 345).given(match, "55.xml").checkTrue(group());
                new Here("Unit", 345).given(match, "52.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "74.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "57.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "50.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "72.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "70.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "48.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "75.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "58.xml").checkTrue(group());
                new Here("Unit", 345).given(match, "53.xml").checkTrue(group());
                new Here("Unit", 345).given(match, "73.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "51.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "71.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "60.xml").checkFalse(group());
                new Here("Unit", 345).given(match, "47.xml").checkFalse(group());
                methodName = match.group(1).trim();
                new Here("Unit", 346).given(match, "62.xml").checkEq(methodName, "maxInboundMessageSize");
                new Here("Unit", 346).given(match, "59.xml").checkEq(methodName, "keepAliveTime");
                new Here("Unit", 346).given(match, "54.xml").checkEq(methodName, "keepAliveWithoutCalls");
                new Here("Unit", 346).given(match, "56.xml").checkEq(methodName, "keepAliveTimeout");
            }
