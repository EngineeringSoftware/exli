...
public class RuntimeContainer {
    ...
    public static void lookupHostname(ErrorReporter errorReporter) {
        ...
        if (!Boolean.parseBoolean(getProperty(PROPERTY_LOGSTASH_GELF_SKIP_HOSTNAME_RESOLUTION, "false"))) {
            try {
                String resolutionOrder = getProperty(PROPERTY_LOGSTASH_GELF_HOSTNAME_RESOLUTION_ORDER, RESOLUTION_ORDER_NETWORK_LOCALHOST_FALLBACK);
    ...
    private static String getHostname(InetAddress inetAddress, boolean fqdn) throws IOException {
        ...
        if (hostname.indexOf('.') != -1 && !fqdn) {
            new Here("Randoop", 121).given(hostname, "rtr-ipaddress.facilities.uu.edu").given(fqdn, true).checkFalse(group());
            new Here("Unit", 121).given(hostname, "hostname").given(fqdn, true).checkFalse(group());
            new Here("Unit", 121).given(hostname, "hostname").given(fqdn, false).checkFalse(group());
            new Here("Unit", 121).given(hostname, "rtr-ipaddress.facilities.uu.edu").given(fqdn, false).checkTrue(group());
            new Here("Randoop", 121).given(hostname, "rtr-ipaddress.facilities.uu.edu").given(fqdn, false).checkTrue(group());
            new Here("Unit", 121).given(hostname, "rtr-ipaddress.facilities.uu.edu").given(fqdn, true).checkFalse(group());
            hostname = hostname.substring(0, hostname.indexOf('.'));
            new Here("Unit", 122).given(hostname, "rtr-ipaddress.facilities.uu.edu").checkEq(hostname, "rtr-ipaddress");
            new Here("Randoop", 122).given(hostname, "rtr-ipaddress.facilities.uu.edu").checkEq(hostname, "rtr-ipaddress");
        }
        return hostname;
    }
