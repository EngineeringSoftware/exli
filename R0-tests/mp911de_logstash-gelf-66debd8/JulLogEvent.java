    ...
    private String createMessage(LogRecord record) {
        ...
        if (parameters != null && parameters.length > 0) {
            ...
            if (message.indexOf('{') != -1) {
                new Here("Unit", 74).given(message, "params {0} and {1}").checkTrue(group());
                new Here("Unit", 74).given(message, "foo bar %s").checkFalse(group());
                new Here("Unit", 74).given(message, "foo bar ''%s''").checkFalse(group());
                new Here("Unit", 74).given(message, "foo bar '%s'").checkFalse(group());
                new Here("Unit", 74).given(message, "%sdfsdfk#! {0}").checkTrue(group());
                new Here("Unit", 74).given(message, "params %s and %d").checkFalse(group());
                new Here("Unit", 74).given(message, "message1").checkFalse(group());
                new Here("Unit", 74).given(message, "Foo {0}").checkTrue(group());
                new Here("Unit", 74).given(message, "foo bar {0}").checkTrue(group());
                new Here("Unit", 74).given(message, "foo bar test log message {0}").checkTrue(group());
                new Here("Unit", 74).given(message, "params {a}").checkTrue(group());
                new Here("Unit", 74).given(message, "params %d").checkFalse(group());
                new Here("Unit", 74).given(message, "foo bar test log message %s").checkFalse(group());
                try {
