...
public class KafkaGelfSenderProvider implements GelfSenderProvider {
    ...
    private static String getBrokerServers(GelfSenderConfiguration configuration) {
        ...
        if (hostsPart.length() > 0) {
            ...
            new Here("Randoop", 93).given(hostsPart, "eConverter").checkEq(hosts, new String[] { "eConverter" });
            new Here("Randoop", 93).given(hostsPart, "LogMessageParameters").checkEq(hosts, new String[] { "LogMessageParameters" });
            new Here("Unit", 93).given(hostsPart, "localhost:9092,localhost:9093").checkEq(hosts, new String[] { "localhost:9092", "localhost:9093" });
            new Here("Unit", 93).given(hostsPart, "localhost").checkEq(hosts, new String[] { "localhost" });
            new Here("Randoop", 93).given(hostsPart, "h-gelf.resolutionOrder").checkEq(hosts, new String[] { "h-gelf.resolutionOrder" });
        }
        if (hosts.length > 0) {
