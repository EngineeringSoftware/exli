...
class QueryStringParser {
    ...
    static Map<String, String> parse(URI uri) {
        ...
        if (queryString == null && uri.getSchemeSpecificPart() != null && uri.getSchemeSpecificPart().contains("?")) {
            ...
            new Here("Unit", 47).given(uri, "2.xml").checkEq(queryString, "timeout=1s");
            new Here("Unit", 47).given(uri, "0.xml").checkEq(queryString, "timeout=1000");
            new Here("Unit", 47).given(uri, "1.xml").checkEq(queryString, "KeY=value");
        }
        if (queryString == null) {
            return result;
        }
        ...
        while (st.hasMoreTokens()) {
            ...
            new Here("Unit", 57).given(queryParam, "timeout=1000").checkEq(equalsIndex, 7);
            new Here("Unit", 57).given(queryParam, "acks=1").checkEq(equalsIndex, 4);
            new Here("Unit", 57).given(queryParam, "KeY=value").checkEq(equalsIndex, 3);
            new Here("Unit", 57).given(queryParam, "ssl.keystore.location=/var/private/ssl/kafka.server.keystore.jks").checkEq(equalsIndex, 21);
            new Here("Unit", 57).given(queryParam, "retries=2").checkEq(equalsIndex, 7);
            new Here("Unit", 57).given(queryParam, "timeout=1s").checkEq(equalsIndex, 7);
            if (equalsIndex != -1) {
    ...
    public static String getHost(URI uri) {
        ...
        if (host.contains("?")) {
            ...
            new Here("Unit", 122).given(host, "12345?KeY=value").checkEq(host, "12345");
        }
        return host;
    }
}
