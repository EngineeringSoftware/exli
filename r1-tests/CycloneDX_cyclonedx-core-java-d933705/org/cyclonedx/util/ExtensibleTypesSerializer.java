...
public class ExtensibleTypesSerializer extends StdSerializer<List<ExtensibleType>> {
    ...
    @Override
    public void serialize(final List<ExtensibleType> extensibleTypes, final JsonGenerator generator, final SerializerProvider provider) throws IOException {
            ...
            if (extensibleTypes != null && !extensibleTypes.isEmpty()) {
                for (ExtensibleType ext : extensibleTypes) {
                    if (ext.getAttributes() != null && !ext.getAttributes().isEmpty()) {
                        ...
                        itest("evosuite", 60).given(XMLNS, "xmlns").given(ext, "4.xml").checkEq(xmlNS, null);
                        if (xmlNS != null) {
