...
class Analysis:
    # python -m analysis proj_to_num_bugs
    ...
    # python -m analysis grouped_inline_tests
    def reduced_inline_tests(self, inline_tests_type: str = "grouped"):
        if inline_tests_type == "grouped":
            log_name = "inlinetest-log-reduced.txt"
        else:
            log_name = "inlinetest-log-all.txt"
        project_names = Util.get_project_names_list_with_sha()
        res = {}
        for project_name, sha in project_names:
            generated_tests_dir = Macros.reduced_tests_dir / f"{project_name}-{sha}"
            if generated_tests_dir.exists():
                # inline tests generated by running tests
                file_to_tests = collections.defaultdict(list)
                if (generated_tests_dir / log_name).exists():
                    with open(generated_tests_dir / log_name, "r") as f:
                        content = f.read()
                        for line in content.splitlines():
                            ...
                            res[key] = {
                                ...
                                "source": "",  # expect to be Unit or Randoop
                            }

                if inline_tests_type != "grouped":
                    generated_tests_dir = Macros.all_tests_dir / f"{project_name}-{sha}"
                    if not generated_tests_dir.exists():
                        continue
                # inline tests that are successfully inserted into the source code
                for test_file in generated_tests_dir.glob("*.java"):
    ...
    # python -m analysis check_mutants_not_killed_all
    ...
    # python -m analysis reduced_inline_tests_missed_mutants
    ...
    # python -m analysis check_mutants_all_not_killed
    ...
    # python -m analysis check_mutants_all_killed_unit_tests_missed
    ...
    # python -m analysis check_mutant_not_killed_tests
    ...
    # python -m analysis source_code_of_not_covered_stmt
